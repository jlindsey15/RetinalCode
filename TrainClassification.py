...
if retina_layers > 2:
    ...
    for iterationX in range(retina_layers - 2):
        if iterationX == retina_layers - 3:
            ...
            conv2_nonlin = Activation(nonlinearity)(conv2)
if retina_layers == 2:
    conv1 = Conv2D(32, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='same', input_shape=x_train.shape[1:], name='conv1', trainable=True)(gn) #conv1 = Dense(8*32*32, kernel_regularizer=keras.regularizers.l1(1.0), activity_regularizer=keras.regularizers.l1(actreg), name='crazy1' )(gn) 
    conv1_relu = Activation(nonlinearity)(conv1)
    ...
    conv2 = Conv2D(1, (filter_size, filter_size), strides=(second_layer_stride,second_layer_stride), kernel_regularizer=keras.regularizers.l1(reg), padding='same',  activity_regularizer=intreg, use_bias=use_b, name='conv2', trainable=True)(conv1_relu)
    conv2_nonlin = Activation(nonlinearity)(conv2)
    gn = ZeroPadding2D((4, 4))(gn)
    crazyconv1 = LocallyConnected2D(32, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='valid', input_shape=x_train.shape[1:], name='crazyconv1', trainable=True)(gn) #conv1 = Dense(8*32*32, kernel_regularizer=keras.regularizers.l1(1.0), activity_regularizer=keras.regularizers.l1(actreg), name='crazy1' )(gn) 
    ...
    #conv2 = Dense(8*32*32, kernel_regularizer=keras.regularizers.l1(reg), activity_regularizer=keras.regularizers.l1(actreg), name='crazy2')(conv1_relu)
    crazyconv2 = LocallyConnected2D(int(sys.argv[17]), (filter_size, filter_size), strides=(second_layer_stride,second_layer_stride), kernel_regularizer=keras.regularizers.l1(reg), padding='valid',  activity_regularizer=intreg, use_bias=use_b, name='crazyconv2', trainable=True)(crazyconv1_relu)
...
if brain_layers == 2:
    ...
    conv3 = Conv2D(64, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='same', name='conv3', trainable=True)(conv2_nonlin)
    conv3_relu = Activation(nonlinearity)(conv3)
    outyou = conv2_nonlin
    
    crazyconv3 = LocallyConnected2D(64, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='valid', name='crazyconv3', trainable=True)(crazyconv2_nonlin)
    print('cc3', crazyconv3.shape)
    crazyconv2_nonlin = Activation(nonlinearity)(crazyconv2)
    crazyconv3_relu = Activation(nonlinearity)(crazyconv3)
    #crazyconv3_relu = ZeroPadding2D((4, 4))(crazyconv3_relu)
    outme = crazyconv3_relu
    #conv4 = Dense(intermediate_dim, kernel_regularizer=keras.regularizers.l1(reg))(conv3_relu)
    conv4 = Conv2D(64, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='same', name='conv4', trainable=True)(conv3_relu)
    conv4_relu = Activation(nonlinearity)(conv4)
    flattened = Flatten()(conv4_relu)
    crazyconv4 = Conv2D(64, (filter_size, filter_size), kernel_regularizer=keras.regularizers.l1(reg), padding='same', name='crazyconv4', trainable=True)(crazyconv3_relu)
    crazyconv4_relu = Activation(nonlinearity)(crazyconv4)
    crazyflattened = Flatten()(crazyconv4_relu)
elif brain_layers == 1:
...
'''
latent = Reshape([32, 32, 64])(flattened)
reconstructAttempt = Conv2DTranspose(1,
                                   kernel_size=num_conv,
                                   padding='same',
                                   strides=1,
                                   activation=nonlinearity)(latent)
print('yum', reconstructAttempt.shape)
y = CustomVariationalLayer()([x, reconstructAttempt])
'''

hidden = Dense(intermediate_dim, kernel_regularizer=keras.regularizers.l1(reg), activity_regularizer=keras.regularizers.l1(actreg), name='dense1', trainable=True)(flattened)
hidden = Activation(nonlinearity)(hidden)
#hidden = GaussianNoise(noise_start)(hidden)
#model.add(Dropout(0.5))
pre_output = Dense(num_classes, name='dense2', trainable=True)(hidden)
output = Activation('softmax')(pre_output)

crazyhidden = Dense(intermediate_dim, kernel_regularizer=keras.regularizers.l1(reg), activity_regularizer=keras.regularizers.l1(actreg), name='crazydense1', trainable=True)(crazyflattened)
crazyhidden = Activation(nonlinearity)(crazyhidden)
#hidden = GaussianNoise(noise_start)(hidden)
#model.add(Dropout(0.5))
crazypre_output = Dense(num_classes, name='crazydense2', trainable=True)(crazyhidden)
crazyoutput = Activation('softmax')(crazypre_output)

z_mean = Conv2D(1, (filter_size, filter_size), strides=(4, 4), kernel_regularizer=keras.regularizers.l1(reg), padding='same')(conv4_relu) #Dense(latent_dim)(flattened)
...
#decoder_upsample = Conv2DTranspose(1, (1, 1), strides=(2, 2), kernel_regularizer=keras.regularizers.l1(reg), padding='same')#Dense(int(filters * NX/2 * NY/2), activation=nonlinearity)
...
decoder_reshape = Conv2DTranspose(filters, (filter_size, filter_size), strides=(2, 2), kernel_regularizer=keras.regularizers.l1(reg), padding='same')#Reshape(output_shape[1:])
...
hid_decoded = z#decoder_hid(z)
up_decoded = hid_decoded#decoder_upsample(hid_decoded)
print(up_decoded.shape, 'seiar')
reshape_decoded = decoder_reshape(up_decoded)
print(reshape_decoded.shape, 'seiar')
if brain_layers > 2:
...
if not os.path.isdir(save_dir):
    os.makedirs(save_dir)
model_name = 'crazyRT0'+sys.argv[17]+model_name
model_path = os.path.join(save_dir, model_name)
