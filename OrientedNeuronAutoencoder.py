...
from keras.layers import Conv2D, MaxPooling2D, Conv2DTranspose, GaussianNoise, UpSampling2D, Input, Lambda
...
#v1var = np.load('V1Variance.npy')
#v1var = np.reshape(v1var, [32, 32, 32])
#v1std = np.sqrt(v1var)
#v1rootcovar = np.load('V1RootCovar.npy')
#v1rootcovar = np.tile(v1rootcovar, [2, 1, 1])
#print(v1rootcovar.shape)
v1samples = np.load('WLC.npy')
v1samples = np.reshape(v1samples, [50000, 32, 32, 32])
filters = 64
NX = 32
NY = 32
NC = 1
...
if retina_layers == 2:
    ...
    conv2 = GaussianNoise(0.0)(Lambda(lambda x: x * K.constant(0))(conv2))
    #conv2 = Flatten()(conv2)
    #thing = Input(shape=[32*32*32, 32*32*32], name='yay')
    v1sample = Input(shape=[32, 32, 32])
    conv2 = keras.layers.Add()([v1sample, conv2])
    #dummy = Reshape([-1])(thing)
    #conv2 = Lambda(lambda x: K.batch_dot(thing, x))(conv2)
    #conv2 = Reshape([32, 32, 32])(conv2)
    print(conv2.shape)
    print('hi')
...
x_train /= 255
x_test /= 255
model_name = 'edgeaebfkcv'+model_name
if True:
    print('Not using data augmentation.')
    if task == 'classification':
            model.fit([x_train, v1samples],
    ...
    if task == 'classification':
        model.fit_generator(datagen.flow([x_train, v1rootcovar], x_train,
                                         batch_size=batch_size),
...
if not os.path.isdir(save_dir):
    os.makedirs(save_dir)
model_name = 'eaebfkWLCm'+model_name
model_path = os.path.join(save_dir, model_name)
